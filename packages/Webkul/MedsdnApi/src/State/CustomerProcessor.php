...
use Webkul\MedsdnApi\Models\Customer;
...
class CustomerProcessor implements ProcessorInterface
{
            ...
            if ($operation->getName() === '_api_/api/shop/customers{._format}_post' || str_contains($operation->getName(), 'post') || $operation->getName() === 'create') {
                ...
                if (! empty($data->password) && ! empty($data->confirm_password)) {
                    if ($data->password !== $data->confirm_password) {
                        throw new InvalidInputException(__('medsdnapi::app.graphql.customer.password-mismatch'));
                    }
                }
                ...
                if ($passwordWasChanged) {
                    if (! isset($data->confirm_password) || empty($data->confirm_password)) {
                        throw new InvalidInputException(__('medsdnapi::app.graphql.customer.confirm-password-required'));
                    }
                    if ($data->password !== $data->confirm_password) {
                        throw new InvalidInputException(__('medsdnapi::app.graphql.customer.password-mismatch'));
                    }
    ...
    private function requireAuthentication(): void
    {
        if (! Auth::guard('sanctum')->check()) {
            throw new AuthenticationException(__('medsdnapi::app.graphql.customer.unauthenticated'));
        }
    }
}
