...
use Webkul\MedsdnApi\Models\Cart as CartModel;
...
class CartProcessor implements ProcessorInterface
{
    ...
    private function handleDestroySelected($data): CartModel
    {
        ...
        if (empty($ids) || ! is_array($ids)) {
            throw new InvalidInputException(__('medsdnapi::app.graphql.cart.select-items-to-remove'));
        }
    ...
    private function handleMoveToWishlist($data): CartModel
    {
        ...
        if (empty($ids) || ! is_array($ids)) {
            throw new InvalidInputException(__('medsdnapi::app.graphql.cart.select-items-to-move-wishlist'));
        }
