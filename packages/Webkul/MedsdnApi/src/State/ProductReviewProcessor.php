...
use Webkul\MedsdnApi\Dto\CreateProductReviewInput;
use Webkul\MedsdnApi\Dto\UpdateProductReviewInput;
...
use Webkul\MedsdnApi\Dto\ProductReviewOutput;
use Webkul\MedsdnApi\Models\ProductReviewAttachment;
...
class ProductReviewProcessor implements ProcessorInterface
{
    ...
    private function handleCreate(CreateProductReviewInput $data)
    {
        ...
        if (! core()->getConfigData('catalog.products.review.customer_review')) {
            throw new AuthorizationException(__('medsdnapi::app.graphql.product-review.review-disabled'));
        }
        ...
        if (! $customer && ! core()->getConfigData('catalog.products.review.guest_review')) {
            throw new AuthorizationException(__('medsdnapi::app.graphql.product-review.guest-review-disabled'));
        }
    ...
    private function validateReview(ProductReview|CreateProductReviewInput|UpdateProductReviewInput $review): void
    {
        ...
        if (empty($productId)) {
            throw new InvalidInputException(__('medsdnapi::app.graphql.product-review.product-id-required'));
        }

        $product = Product::find($productId);
        if (! $product) {
            throw new ResourceNotFoundException(__('medsdnapi::app.graphql.product-review.product-not-found'));
        }
        ...
        if (isset($rating)) {
            ...
            if ($ratingVal < 1 || $ratingVal > 5) {
                throw new InvalidInputException(__('medsdnapi::app.graphql.product-review.rating-invalid'));
            }
        }
        ...
        if ($review instanceof CreateProductReviewInput) {
            if (empty($title)) {
                throw new InvalidInputException(__('medsdnapi::app.graphql.product-review.title-required'));
            }
            if (empty($comment)) {
                throw new InvalidInputException(__('medsdnapi::app.graphql.product-review.comment-required'));
            }
        }
    }
