...
class CartMutation extends Controller
{
    ...
    public function store(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        try {
            ...
            $data = medsdn_graphql()->manageInputForCart($product, $args);
            ...
            return [
                'success' => ! empty($cart),
                'message' => ! empty($cart)
                    ? trans('medsdn_graphql::app.shop.checkout.cart.item.success.add-to-cart')
                    : trans('medsdn_graphql::app.shop.checkout.cart.item.fail.add-to-cart'),
                'cart'    => $cart,
            ];
        } catch (\Exception $e) {
            throw new CustomException($e->getMessage());
        ...
        try {
            ...
            foreach ($args['qty'] as $item) {
                if (! $this->cartItemRepository->find($item['cart_item_id'])) {
                    throw new CustomException(trans('medsdn_graphql::app.shop.checkout.cart.item.fail.item-not-found'));
                }
            ...
            return [
                'success' => $cartUpdated,
                'message' => $cartUpdated
                    ? trans('medsdn_graphql::app.shop.checkout.cart.item.success.update-to-cart')
                    : trans('medsdn_graphql::app.shop.checkout.cart.item.fail.update-to-cart'),
                'cart'    => Cart::getCart(),
            ];
        } catch (\Exception $e) {
            throw new CustomException($e->getMessage());
    ...
    public function delete(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        try {
            ...
            return [
                'success' => $isRemoved,
                'message' => $isRemoved
                    ? trans('medsdn_graphql::app.shop.checkout.cart.item.success.delete-cart-item')
                    : trans('medsdn_graphql::app.shop.checkout.cart.item.fail.delete-cart-item'),
                'cart'    => Cart::getCart(),
            ];
        } catch (\Exception $e) {
            throw new CustomException($e->getMessage());
    ...
    public function deleteAll(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        if (! $cart) {
            throw new CustomException(trans('medsdn_graphql::app.shop.checkout.cart.item.fail.not-found'));
        }
        ...
        try {
            ...
            return [
                'success' => $isDeleted,
                'message' => $isDeleted
                    ? trans('medsdn_graphql::app.shop.checkout.cart.item.success.all-remove')
                    : trans('medsdn_graphql::app.shop.checkout.cart.item.fail.all-remove'),
            ];
        } catch (\Exception $e) {
            throw new CustomException($e->getMessage());
        }
    ...
    public function moveToWishlist(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        try {
            ...
            return [
                'success' => $isMoved,
                'message' => $isMoved
                    ? trans('medsdn_graphql::app.shop.checkout.cart.item.success.move-to-wishlist')
                    : trans('medsdn_graphql::app.shop.checkout.cart.item.fail.move-to-wishlist'),
                'cart'    => Cart::getCart(),
            ];
        } catch (\Exception $e) {
            throw new CustomException($e->getMessage());
