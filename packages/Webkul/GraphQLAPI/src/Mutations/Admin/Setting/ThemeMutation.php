...
class ThemeMutation extends Controller
{
    ...
    public function store(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        return [
            'success' => true,
            'message' => trans('medsdn_graphql::app.admin.settings.themes.create-success'),
            'theme'   => $theme,
        ];
    }
    ...
    public function update(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        if (! $themeCustomization) {
            throw new CustomException(trans('medsdn_graphql::app.admin.settings.themes.not-found'));
        }
                ...
                'options.filtersInput' => [
                    function ($attribute, $value, $fail) use ($allowedSortOptions, $allowedLimitOptions) {
                        ...
                        if (! in_array('sort', $keys)) {
                            $fail(trans('medsdn_graphql::app.admin.settings.themes.validation.filter-input.missing-sort-key'));
                        } elseif (! in_array($valuesByKey['sort'], $allowedSortOptions)) {
                            $fail(trans('medsdn_graphql::app.admin.settings.themes.validation.filter-input.invalid-sort-value', ['options' => implode(', ', $allowedSortOptions)]));
                        }

                        if (! in_array('limit', $keys)) {
                            $fail(trans('medsdn_graphql::app.admin.settings.themes.validation.filter-input.missing-limit-key'));
                        } elseif (! in_array($valuesByKey['limit'], $allowedLimitOptions)) {
                            $fail(trans('medsdn_graphql::app.admin.settings.themes.validation.filter-input.invalid-limit-value', ['options' => implode(', ', $allowedLimitOptions)]));
                        }
                        ...
                        foreach ($keys as $key) {
                            if (
                                ...
                                if (! in_array($key, $allowedFilterKeys)) {
                                    $fail(trans('medsdn_graphql::app.admin.settings.themes.validation.filter-input.invalid-filter-key', ['key' => $key]));
                                }

                                if ($key === 'category_id') {
                                    if (! $this->categoryRepository->find($valuesByKey[$key])) {
                                        $fail(trans('medsdn_graphql::app.admin.settings.themes.validation.filter-input.category-not-exist'));
                                    }
                                }
                                ...
                                if (
                                    ...
                                    if (! in_array($valuesByKey[$key], [0, 1])) {
                                        $fail(trans('medsdn_graphql::app.admin.settings.themes.validation.filter-input.invalid-boolean-value', ['key' => $key]));
                                    }
                                }
                                    ...
                                    switch ($attribute->type) {
                                        ...
                                        case 'multiselect':
                                            ...
                                            if (! in_array($valuesByKey[$key], $validOptions)) {
                                                $fail(trans('medsdn_graphql::app.admin.settings.themes.validation.filter-input.invalid-select-option', ['key' => $key, 'options' => implode(', ', $validOptions)]));
                                            }
                                            break;
        ...
        return [
            'success' => true,
            'message' => trans('medsdn_graphql::app.admin.settings.themes.update-success'),
            'theme'   => $theme,
        ];
    }
    ...
    public function delete(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        return [
            'success' => true,
            'message' => trans('medsdn_graphql::app.admin.settings.themes.delete-success'),
        ];
    }
}
