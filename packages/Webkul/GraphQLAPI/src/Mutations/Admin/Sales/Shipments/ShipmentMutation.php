...
class ShipmentMutation extends Controller
{
    ...
    public function store(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        if (! $order->canShip()) {
            throw new CustomException(trans('medsdn_graphql::app.admin.sales.shipments.shipment-error'));
        }

        try {
            if (! isset($args['shipment_data'])) {
                throw new CustomException(trans('medsdn_graphql::app.admin.sales.shipments.creation-error'));
            }
            ...
            medsdn_graphql()->validate($shipment, [
                'shipment.source'    => 'required',
                'shipment.items.*.*' => 'required|numeric|min:0',
            ]);

            if (! $this->isInventoryValidate($shipment)) {
                throw new CustomException(trans('medsdn_graphql::app.admin.sales.shipments.quantity-invalid'));
            }
            ...
            return [
                'success'  => true,
                'message'  => trans('medsdn_graphql::app.admin.sales.shipments.create-success'),
                'shipment' => $shipment,
            ];
        } catch (\Exception $e) {
