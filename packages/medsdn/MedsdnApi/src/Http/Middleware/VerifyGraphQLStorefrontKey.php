...
namespace Webkul\MedsdnApi\Http\Middleware;
...
use Webkul\MedsdnApi\Attributes\AllowPublic;
use Webkul\MedsdnApi\Attributes\RequiresStorefrontKey;
use Webkul\MedsdnApi\Services\ApiKeyService;
...
<script>
(function() {
    const storefrontKey = localStorage.getItem('medsdn-api-key') || 'JS' . $storefrontKey . 'JS';
    ...
    window.setMedsdnApiKey = function(key) {
        localStorage.setItem('medsdn-api-key', key);
        console.log('MedSDN API Key updated:', key);
    };

    window.getMedsdnApiKey = function() {
        return localStorage.getItem('medsdn-api-key') || storefrontKey;
    };

    console.log('%cMedSDN GraphQL API', 'color: #667eea; font-size: 14px; font-weight: bold;');
    console.log('X-STOREFRONT-KEY: ' + getMedsdnApiKey());
    console.log('Change key with: setMedsdnApiKey("your-key-here")');
})();
</script>
JS;
