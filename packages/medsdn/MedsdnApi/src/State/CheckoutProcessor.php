...
class CheckoutProcessor implements ProcessorInterface
{
    ...
    public function process(
        ...
        if (! $data instanceof CheckoutAddressInput) {
            throw new OperationFailedException(__('medsdnapi::app.graphql.checkout.invalid-input'));
        }
        ...
        return match ($operationName) {
            ...
            default              => throw new OperationFailedException(__('medsdnapi::app.graphql.checkout.unknown-operation', ['operation' => $operationName])),
        };
    }

    /**
     * Map MedsdnApi operation name to internal operation type
     */
    ...
    private function saveAddress($cart, CheckoutAddressInput $input)
    {
        try {
            if (! $input->billingFirstName && ! $input->billingAddress) {
                throw new OperationFailedException(__('medsdnapi::app.graphql.checkout.billing-address-required'));
            }

            if ($cart->haveStockableItems()) {
                $hasShippingData = $input->shippingFirstName || $input->shippingAddress || $input->useForShipping;
                if (! $hasShippingData) {
                    throw new OperationFailedException(__('medsdnapi::app.graphql.checkout.shipping-address-required'));
                }
            }
    ...
    private function saveShippingMethod($cart, CheckoutAddressInput $input)
    {
        try {
            ...
            if (! $input->shippingMethod) {
                throw new OperationFailedException(__('medsdnapi::app.graphql.checkout.shipping-method-required'));
            }

            \Webkul\Shipping\Facades\Shipping::collectRates();

            if (! \Webkul\Shipping\Facades\Shipping::isMethodCodeExists($input->shippingMethod)) {
                throw new OperationFailedException(__('medsdnapi::app.graphql.checkout.invalid-shipping-method'));
            }

            if (! \Webkul\Checkout\Facades\Cart::saveShippingMethod($input->shippingMethod)) {
                throw new OperationFailedException(__('medsdnapi::app.graphql.checkout.shipping-method-save-failed'));
            }
            ...
            return (object) [
                'id'             => (string) $cart->id,
                'success'        => true,
                'message'        => __('medsdnapi::app.graphql.checkout.shipping-method-saved'),
    ...
    private function savePaymentMethod($cart, CheckoutAddressInput $input)
    {
        ...
        if (! $paymentMethodConfig || ! isset($paymentMethodConfig['class'])) {
            throw new OperationFailedException(__('medsdnapi::app.graphql.checkout.invalid-payment-method'));
        }

        if (! \Webkul\Checkout\Facades\Cart::savePaymentMethod(['method' => $input->paymentMethod])) {
            throw new OperationFailedException(__('medsdnapi::app.graphql.checkout.payment-method-save-failed'));
        }
        ...
        try {
            ...
            $response = (object) [
                'success'        => true,
                'message'        => __('medsdnapi::app.graphql.checkout.payment-method-saved'),
                'cartToken'      => (string) ($cart->guest_cart_token ?? $cart->customer_id),
    ...
    private function createOrder($cart, CheckoutAddressInput $input)
    {
        try {
            ...
            if (! $order || ! $order->id) {
                throw new \Exception(__('medsdnapi::app.graphql.checkout.order-creation-failed'));
            }
            ...
            if (! $order) {
                throw new \Exception(__('medsdnapi::app.graphql.checkout.order-retrieval-failed', ['orderId' => $orderId]));
            }
    ...
    private function validateOrderCreation($cart, CheckoutAddressInput $input): void
    {
        if (! $cart || $cart->items()->count() === 0) {
            throw new OperationFailedException(__('medsdnapi::app.graphql.checkout.cart-empty'));
        }
        ...
        if (auth()->guard('customer')->check()) {
            ...
            if ($customer && $customer->is_suspended) {
                throw new OperationFailedException(__('medsdnapi::app.graphql.checkout.account-suspended'));
            }

            if ($customer && ! $customer->status) {
                throw new OperationFailedException(__('medsdnapi::app.graphql.checkout.account-inactive'));
            }
        }
        ...
        if (! \Webkul\Checkout\Facades\Cart::haveMinimumOrderAmount()) {
            throw new OperationFailedException(__('medsdnapi::app.graphql.checkout.minimum-order-not-met', ['amount' => core()->currency($minimumOrderAmount)]));
        }
        ...
        if (! $hasEmail) {
            throw new OperationFailedException(__('medsdnapi::app.graphql.checkout.email-required'));
        }
    ...
    private function buildAddressOutput($billingAddress = null, $shippingAddress = null)
    {
        $output = (object) [
            'success' => true,
            'message' => __('medsdnapi::app.graphql.checkout.address-saved'),
        ];
    ...
    {
        try {
            $output = new \Webkul\MedsdnApi\Dto\CheckoutAddressOutput;
