...
use Webkul\MedsdnApi\Models\CustomerDownloadableProduct;
...
class CustomerDownloadableProductProvider implements ProviderInterface
{
    ...
    private function provideItem(object $customer, array $uriVariables): CustomerDownloadableProduct
    {
        ...
        if (! $id) {
            throw new ResourceNotFoundException(__('medsdnapi::app.graphql.customer-downloadable-product.id-required'));
        }
        ...
        if (! $item) {
            throw new ResourceNotFoundException(
                __('medsdnapi::app.graphql.customer-downloadable-product.not-found', ['id' => $id])
            );
        }
