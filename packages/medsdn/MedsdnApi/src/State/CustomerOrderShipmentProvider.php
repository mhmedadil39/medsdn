...
class CustomerOrderShipmentProvider implements ProviderInterface
{
    ...
    private function provideItem(object $customer, array $uriVariables): CustomerOrderShipment
    {
        ...
        if (! $id) {
            throw new ResourceNotFoundException(__('medsdnapi::app.graphql.customer-order-shipment.id-required'));
        }
        ...
        if (! $shipment) {
            throw new ResourceNotFoundException(
                __('medsdnapi::app.graphql.customer-order-shipment.not-found', ['id' => $id])
            );
        }
