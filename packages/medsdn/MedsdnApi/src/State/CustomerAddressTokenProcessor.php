...
class CustomerAddressTokenProcessor implements ProcessorInterface
{
    public function process(
        ...
        if (! $token) {
            throw new AuthenticationException(__('medsdnapi::app.graphql.address.authentication-required'));
        }
        ...
        if (! $customer) {
            throw new AuthenticationException(__('medsdnapi::app.graphql.address.invalid-token'));
        }
        ...
        return match ($operationName) {
            ...
            default        => throw new InvalidInputException(__('medsdnapi::app.graphql.address.unknown-operation')),
        };
    }
    ...
    {
        if (! $data->addressId) {
            throw new InvalidInputException(__('medsdnapi::app.graphql.address.address-id-required'));
        }

        $address = AddressModel::find($data->addressId);
        if (! $address || $address->customer_id !== $customer->id) {
            throw new AuthorizationException(__('medsdnapi::app.graphql.address.address-not-found'));
        }
