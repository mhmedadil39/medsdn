...
use Webkul\MedsdnApi\Dto\ReorderInput;
...
use Webkul\MedsdnApi\Models\ReorderOrder;
...
class ReorderProcessor implements ProcessorInterface
{
     ...
     *
     * Business logic follows MedSDN's native reorder flow:
    ...
    private function handleReorder(ReorderInput $input): ReorderOrder
    {
        ...
        if (empty($input->orderId)) {
            throw new InvalidInputException(__('medsdnapi::app.graphql.reorder.order-id-required'));
        }
        ...
        if (! $order) {
            throw new ResourceNotFoundException(
                __('medsdnapi::app.graphql.reorder.not-found', ['id' => $input->orderId])
            );
        }
        ...
        if ($itemsAdded > 0) {
            return new ReorderOrder(
                success: true,
                message: __('medsdnapi::app.graphql.reorder.reorder-success', ['count' => $itemsAdded]),
                orderId: $order->id,
        ...
        return new ReorderOrder(
            success: false,
            message: __('medsdnapi::app.graphql.reorder.no-items-added'),
            orderId: $order->id,
            itemsAddedCount: 0,
        );
    }
