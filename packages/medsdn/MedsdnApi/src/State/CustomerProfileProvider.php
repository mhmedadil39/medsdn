...
class CustomerProfileProvider implements ProviderInterface
{
    public function provide(Operation $operation, array $uriVariables = [], array $context = []): object|array|null
    {
        ...
        if (! $token) {
            throw new AuthenticationException(__('medsdnapi::app.graphql.customer-profile.authentication-required'));
        }

        $authenticatedCustomer = $this->getCustomerFromToken($token);

        if (! $authenticatedCustomer) {
            throw new AuthenticationException(__('medsdnapi::app.graphql.customer-profile.invalid-token'));
        }
