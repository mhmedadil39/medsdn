 ...
 */
class SingleProductMedsdnApiResolver extends BaseQueryItemResolver implements QueryItemResolverInterface
{
    /**
    ...
    public function __invoke(?object $item, array $context): object
    {
        ...
        $locale  = $args['locale'] ?? request()->attributes->get('medsdn_locale');
        $channel = $args['channel'] ?? request()->attributes->get('medsdn_channel');
        ...
        throw new InvalidInputException(__('medsdnapi::app.graphql.product.missing-query-parameter'));
    }
    ...
    private function resolveById(int|string $id): Product
    {
        ...
        if (! $product) {
            throw new ResourceNotFoundException(__('medsdnapi::app.graphql.product.not-found'));
        }
    ...
    private function resolveBySku(string $sku): Product
    {
        return Product::where('sku', $sku)
            ->first() ?? throw new ResourceNotFoundException(
                __('medsdnapi::app.graphql.product.not-found-with-sku')
            );
    }
    ...
    private function resolveByUrlKey(string $urlKey): Product
    {
        ...
        if (! $product) {
            throw new ResourceNotFoundException(
                __('medsdnapi::app.graphql.product.not-found-with-url-key')
            );
        }
