...
use Webkul\MedsdnApi\Service\GenericIdNormalizer;
...
class BaseQueryItemResolver implements QueryItemResolverInterface
{
     ...
     * - "Resource" -> "Webkul\MedsdnApi\Models\Resource"
     *
     * @param  array  $context  GraphQL resolver context
    ...
    protected function getModel(array $context): string
    {
        ...
        $modelClass = "Webkul\\MedsdnApi\\Models\\{$resourceClass}";

        if (! class_exists($modelClass)) {
            throw new BadRequestHttpException(
                "Model class {$modelClass} not found for GraphQL type {$resourceClass}. ".
                'Please ensure the model exists in Webkul\\MedsdnApi\\Models namespace.'
            );
        }
     ...
     * - Resource -> medsdnapi::app.graphql.resource
     * - ProductBundleOption -> medsdnapi::app.graphql.product-bundle-option
     *
    ...
    protected function getErrorMessagePrefix(): string
    {
        if ($this->currentResourceClass) {
            return 'medsdnapi::app.graphql.'.$this->classToKebabCase($this->currentResourceClass);
        }

        return 'medsdnapi::app.graphql.unknown-resource';
    }
