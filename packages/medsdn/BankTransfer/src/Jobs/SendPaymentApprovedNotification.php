...
class SendPaymentApprovedNotification implements ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            if (!$customerEmail) {
                Log::warning('Cannot send payment approved notification: customer email not found', [
            ...
            Mail::to($customerEmail)->send(
                new \Webkul\BankTransfer\Mail\PaymentApproved($this->payment)
            );

            // Log notification sent
            Log::info('Payment approved notification sent to customer', [
                'payment_id' => $this->payment->id,
                'order_id' => $this->payment->order_id,
            ]);
        } catch (\Exception $e) {
            // Log error but don't fail the job
            Log::error('Failed to send payment approved notification', [
                'payment_id' => $this->payment->id,
                'order_id' => $this->payment->order_id,
                'error' => $e->getMessage(),
            ]);
