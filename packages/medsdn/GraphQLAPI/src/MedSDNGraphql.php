...
class MedSDNGraphql
{
    ...
    public function authorize(string $guard = 'api', ?string $token = null): mixed
    {
        if (! auth()->guard($guard)->check()) {
            throw new CustomException(trans('medsdn_graphql::app.shop.customers.no-login-customer'));
        }
        ...
        if (
            ...
            $message = trans('medsdn_graphql::app.shop.customers.login.suspended');
        }
        ...
        switch ($product->type) {
            ...
            case 'downloadable':
                if (! empty($data['links'])) {
                    ...
                    if (empty($data['links'])) {
                        throw new CustomException(trans('medsdn_graphql::app.shop.checkout.cart.item.error.downloadable-links'));
                    }
                }
                break;
