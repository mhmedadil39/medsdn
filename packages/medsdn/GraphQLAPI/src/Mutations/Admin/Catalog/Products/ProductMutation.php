...
class ProductMutation extends Controller
{
    ...
    public function store(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        if (
            ...
            throw new CustomException(trans('medsdn_graphql::app.admin.catalog.products.create.configurable-attr-missing'));
        }
        ...
        try {
            ...
            return [
                'success' => true,
                'message' => trans('medsdn_graphql::app.admin.catalog.products.create-success'),
                'product' => $product,
            ];
        } catch (\Exception $e) {
    ...
    public function update(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        if (! $product) {
            throw new CustomException(trans('medsdn_graphql::app.admin.catalog.products.not-found'));
        }
        ...
        if (
            ...
            $args['variants'] = medsdn_graphql()->manageConfigurableRequest($args);
        }

        if ($product->type == 'booking') {
            if (! empty($args['booking'])) {
                $args['booking'] = medsdn_graphql()->manageBookingRequest($product, $args['booking']);
            }
        }
        ...
        if (
            ...
            foreach ($args['links'] as $linkProduct) {
                ...
                if (
                    ...
                    throw new CustomException(trans('medsdn_graphql::app.admin.catalog.products.simple-products-error'));
                }
            }

            $args['links'] = medsdn_graphql()->manageGroupedRequest($product, $args);
        }

        if ($product->type == 'downloadable') {
            if (! empty($args['downloadable_links'])) {
                $args['downloadable_links'] = medsdn_graphql()->manageDownloadableLinksRequest($product, $args);
            }

            if (! empty($args['downloadable_samples'])) {
                $args['downloadable_samples'] = medsdn_graphql()->manageDownloadableSamplesRequest($product, $args);
            }
        }
        ...
        if (
            ...
            $args['bundle_options'] = medsdn_graphql()->manageBundleRequest($product, $args);
        }

        if (! empty($args['customer_group_prices'])) {
            $args['customer_group_prices'] = medsdn_graphql()->manageCustomerGroupPrices($product, $args);
        }

        if (! empty($args['customizable_options'])) {
            $args['customizable_options'] = medsdn_graphql()->manageCustomizableOptions($product, $args);
        }
        ...
        try {
            ...
            if ($product) {
                ...
                medsdn_graphql()->uploadProductImages($uploadParams);

                medsdn_graphql()->uploadProductImages(array_merge($uploadParams, [
                    'data'      => $videoUrls,
                    'data_type' => 'videos',
                ]));
            }
            ...
            return [
                'success' => true,
                'message' => trans('medsdn_graphql::app.admin.catalog.products.update-success'),
                'product' => $product,
            ];
        } catch (\Exception $e) {
            throw new CustomException($e->getMessage());
    ...
    public function delete(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        try {
            ...
            return [
                'success' => true,
                'message' => trans('medsdn_graphql::app.admin.catalog.products.delete-success'),
            ];
        } catch (\Exception $e) {
            throw new CustomException($e->getMessage());
        }
