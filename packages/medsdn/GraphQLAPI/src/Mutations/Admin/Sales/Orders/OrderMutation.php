...
class OrderMutation extends Controller
{
    ...
    public function cancel(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        if (
            ...
            throw new CustomException(trans('medsdn_graphql::app.admin.sales.orders.cancel-error'));
        }
        ...
        try {
            ...
            return [
                'success' => $result,
                'message' => $result
                    ? trans('medsdn_graphql::app.admin.sales.orders.cancel-success')
                    : trans('medsdn_graphql::app.admin.sales.orders.cancel-error'),
                'order'   => $this->orderRepository->find($args['id']),
            ];
        } catch (\Exception $e) {
