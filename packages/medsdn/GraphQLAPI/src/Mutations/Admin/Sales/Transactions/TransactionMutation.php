...
class TransactionMutation extends Controller
{
    ...
    public function store(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        if (! $invoice) {
            throw new CustomException(trans('medsdn_graphql::app.admin.sales.invoices.not-found'));
        }
        ...
        if ($invoice->state == 'paid') {
            throw new CustomException(trans('medsdn_graphql::app.admin.sales.transactions.already-paid'));
        }

        if ($transactionAmtFinal > $invoice->base_grand_total) {
            throw new CustomException(trans('medsdn_graphql::app.admin.sales.transactions.amount-exceed'));
        }

        if ($args['amount'] <= 0) {
            throw new CustomException(trans('medsdn_graphql::app.admin.sales.transactions.zero-amount'));
        }
        ...
        return [
            'success'     => true,
            'message'     => trans('medsdn_graphql::app.admin.sales.transactions.zero-amount'),
