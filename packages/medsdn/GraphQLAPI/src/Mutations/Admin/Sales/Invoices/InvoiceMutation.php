...
class InvoiceMutation extends Controller
{
    ...
    public function store(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        if (! $order->canInvoice()) {
            throw new CustomException(trans('medsdn_graphql::app.admin.sales.invoices.creation-error'));
        }

        try {
            if (empty($args['invoice_data'])) {
                throw new CustomException(trans('medsdn_graphql::app.admin.sales.invoices.product-error'));
            }
            ...
            if (! $this->invoiceRepository->isValidQuantity($invoice)) {
                throw new CustomException(trans('medsdn_graphql::app.admin.sales.invoices.invalid-qty'));
            }
            ...
            return [
                'success'  => true,
                'message'  => trans('medsdn_graphql::app.admin.sales.invoices.create-success'),
                'invoice'  => $invoice,
            ];
        } catch (\Exception $e) {
