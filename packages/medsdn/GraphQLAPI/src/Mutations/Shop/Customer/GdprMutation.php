...
class GdprMutation extends Controller
{
    ...
    public function store(mixed $rootValue, array $args, GraphQLContext $context)
    {
        if (core()->getConfigData('general.gdpr.settings.enabled') != '1') {
            throw new CustomException(trans('medsdn_graphql::app.shop.customers.account.gdpr.not-enabled'));
        }
        ...
        try {
            ...
            return [
                'status'       => true,
                'message'      => trans('medsdn_graphql::app.shop.customers.account.gdpr.create-success'),
    ...
    public function revoke($rootValue, array $args, GraphQLContext $context)
    {
        ...
        if (! $data) {
            return [
                'status'  => false,
                'message' => trans('medsdn_graphql::app.shop.customers.account.gdpr.revoke-failed'),
            ];
        }
        ...
        return [
            'status'       => true,
            'message'      => trans('medsdn_graphql::app.shop.customers.account.gdpr.revoked-successfully'),
            'gdprRequest'  => $gdprRequest,
        ];
    }
