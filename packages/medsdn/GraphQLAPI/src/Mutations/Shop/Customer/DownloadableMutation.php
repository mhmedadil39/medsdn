...
class DownloadableMutation extends Controller
{
    ...
    public function download(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        if (
            ...
            throw new CustomException(trans('medsdn_graphql::app.shop.customers.account.downloadable-products.not-auth'));
        }

        if ($downloadableLinkPurchased->status == 'expired') {
            throw new CustomException(trans('medsdn_graphql::app.shop.customers.account.downloadable-products.download-error'));
        }
        ...
        if (
            ...
            throw new CustomException(trans('medsdn_graphql::app.shop.customers.account.downloadable-products.payment-error'));
        }
